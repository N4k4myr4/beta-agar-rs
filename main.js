...
class Client {
    ...
    async fetchLatest() {
        const file = await fetch("https://beta.agar.rs/assets/js/main.min.js?vfb=00000500537").then((response) => response.text());
    ...
    startBots(amount) {
        ...
        for (let i = 0; i < amount; i++) {
            this.bots.push(new Bot(this.protocolKey, window.client.botID, `wss://game2.agar.rs:443`, false));
            this.botID++;
        }
